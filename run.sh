date
./compile.sh
#time ./pagerank 2 < tests/sample.in | diff - tests/sample.out
...
#valgrind --show-leak-kinds=all --leak-check=full ./pagerank 4 < tests/cdog-medium.in | diff - tests/cdog-medium.out
gprof pagerank
#head tests/cdog-medium.out
